...
## 📝 技術筆記：系統架構設計
本專案在設計上特別強調系統彈性與效能優化：
1. **運算與存取功能拆分**：為了保有系統的彈性，我將**運算（排序）與存取（匯出）**功能拆分。使用者可以透過互動式選單先選定所需的數據維度進行預覽，確認無誤後再執行資料持久化（匯出）。
2. **I/O 效能優化**：此設計可避免因頻繁的磁碟 I/O 導致系統效能下降，同時也賦予使用者自主權，決定是否保留原始數據排列或產出特定維度的報表。
